...
class Validators::Barter::ReceivenItemValidator < ActiveModel::Validator
  def validate(record)
    ...
    if record.receiven_items[:items].nil? || record.receiven_items[:items].empty?
      return record.errors.add(:receiven_items,
                               :is_empty)
    end
    ...
    if record.receiven_items[:user_id] == record.given_items[:user_id]
      return record.errors.add(:receiven_items,
                               :equal_users)
    end
